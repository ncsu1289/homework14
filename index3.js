...
function handleSearchButtonClick() {
  ...
  searchData = dataSet.filter(function(address) {
    ...
    var addressState = address.state.substring(0, filterShape.length).toLowerCase();
    var addressCity = address.city.substring(0, filterShape.length).toLowerCase();
    var addressCountry = address.country.substring(0, filterShape.length).toLowerCase();
    var addressShape = address.shape.substring(0, filterShape.length).toLowerCase();
    if (addressDate === filterDate && addressState === filterState && addressCity === filterCity && addressShape === filterShape && addressCountry === filterCountry){
      return true;
    }
